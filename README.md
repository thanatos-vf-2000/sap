# Ansible Collection - thanatos_vf_2000.sap

This collection contains components to be used with SAP NetWeaver
hosted in [Ansible Galaxy](https://galaxy.ansible.com/thanatos_vf_2000/sap/).

## Table of Contents

  * [Requirements](#requirements)
  * [Installation](#installation)
  * [Contributing](#contributing)
  * [License](#license)

<!-- Table of contents generated with markdown-toc
http://ecotrust-canada.github.io/markdown-toc/ -->

## Requirements

- Ansible >= 2.13
- SAP HostAgent install on SAP servers.

## Installation 

From terminal, run the following command:
```sh
ansible-galaxy collection install thanatos_vf_2000.sap
```

## Contributing

We welcome contributions of all kinds to this repository. For instructions on how to get started and
descriptions of our development workflows, please see our [contributing guide][contrib].

[contrib]: https://github.com/thanatos-vf-2000/sap/blob/main/CONTRIBUTING.md

## License

Copyright (c) 2025 thanatos-vf-2000. All rights reserved.

Licensed under the GPL 3.0 or later; you may not use this file except in 
compliance with the License. You may obtain a copy of the License at

   https://www.gnu.org/licenses/gpl-3.0.html


For the full license text see [`LICENSE`](gpl-3.0).
